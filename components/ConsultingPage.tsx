...
const ConsultingPage: React.FC = () => {
  ...
  const categories = [
    { id: 'cons_tech', icon: 'tech', color: 'bg-blue-50 text-blue-600', border: 'hover:border-blue-500', type: 'technical' },
    { id: 'cons_marketing', icon: 'market', color: 'bg-purple-50 text-purple-600', border: 'hover:border-purple-500', type: 'growth' },
    { id: 'cons_training', icon: 'train', color: 'bg-green-50 text-green-600', border: 'hover:border-green-500', type: 'hr' },
    { id: 'cons_recruitment', icon: 'recruit', color: 'bg-orange-50 text-orange-600', border: 'hover:border-orange-500', type: 'hr' },
    { id: 'cons_gov', icon: 'gov', color: 'bg-slate-50 text-slate-600', border: 'hover:border-slate-500', type: 'legal' },
  ];
         ...
         <div className="bg-white rounded-[32px] shadow-elevated p-8 md:p-12 border border-slate-100">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
               ...
               <div className="lg:col-span-5 flex flex-col h-full">
                  ...
                  {/* AI Assistant Block */}
                  <div className="mb-8 bg-gradient-to-br from-blue-50 to-indigo-50 p-6 rounded-2xl border border-blue-100/50 relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-24 h-24 bg-brand-primary/5 rounded-full blur-2xl -mr-10 -mt-10 pointer-events-none"></div>
                        
                        <div className="flex items-center gap-2 mb-3">
                            <div className="w-8 h-8 rounded-lg bg-white flex items-center justify-center text-brand-primary shadow-sm">
                                ✨
                            </div>
                            <h4 className="font-bold text-brand-primary text-sm">{t('aiRecommendation')}</h4>
                        </div>
                        ...
                        <div className="relative">
                            <textarea
                                ...
                                className="w-full p-3 rounded-xl border border-blue-200 text-sm focus:ring-2 focus:ring-blue-500/20 outline-none bg-white min-h-[80px] resize-none text-brand-primary placeholder:text-slate-400"
                            />
                            <button 
                                onClick={handleAiRecommendation}
                                disabled={isAnalyzing || !aiQuery.trim()}
                                className="absolute bottom-2 right-2 p-2 bg-brand-primary text-white rounded-lg hover:bg-brand-accent transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
                            >
                                {isAnalyzing ? (
                        ...
                        {aiReasoning && (
                            <div className="mt-4 p-3 bg-white/80 rounded-xl border border-blue-100 text-xs text-brand-primary leading-relaxed animate-fade-in flex gap-2 items-start">
                  ...
                  <div className="space-y-4 flex-1 overflow-y-auto max-h-[600px] pr-2 custom-scrollbar">
                     {filteredCategories.length === 0 ? (
                       <div className="text-center py-8 text-slate-400 text-sm">No services match this filter.</div>
                     ) : (
